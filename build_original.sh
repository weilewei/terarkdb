...
if [ "$WITH_TESTS" == "1" ]; then
  ...
  WITH_TESTS=OFF
  echo "build $BUILD_TYPE, with_tests = $WITH_TESTS"
  cd $BASE/$OUTPUT && cmake ../ -DCMAKE_INSTALL_PREFIX=$OUTPUT -DCMAKE_BUILD_TYPE=Release -DWITH_TOOLS=ON -DWITH_TERARK_ZIP=ON -DWITH_ZENFS=ON -DWITH_ZENFS_ASYNC_METAZONE_ROLLOVER=OFF
  cd $BASE/$OUTPUT && make -j $(nproc) 
fi
